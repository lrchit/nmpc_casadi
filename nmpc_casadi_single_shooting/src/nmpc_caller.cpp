...
void nmpc_caller::set_nmpc_solver() {
  ...
  // 1. 状态损失，末端损失权重更大
  SX x_next = Eular_method(x_cur, u(Slice(0, 12)), p_foot);
  SX states_err = x_next - x_ref(Slice(0, 12));
  f = SX::mtimes({states_err.T(), weightQ, states_err});
  for (int k = 1; k < horizon - 1; k++) {
    x_next = Eular_method(x_next, u(Slice(12 * k, 12 * (k + 1))), p_foot);
    states_err = x_next - x_ref(Slice(k * 12, (k + 1) * 12));
    f = f + SX::mtimes({states_err.T(), weightQ, states_err});
  }
  x_next =
      Eular_method(x_next, u(Slice(12 * (horizon - 1), 12 * horizon)), p_foot);
  states_err = x_next - x_ref(Slice((horizon - 1) * 12, horizon * 12));
  f = f + SX::mtimes({states_err.T(), weightQ_t, states_err});
  // 2. 控制输入的正则化
  for (int k = 0; k < horizon; k++) {
    SX controls_val = u(Slice(12 * k, 12 * (k + 1)));
  ...
  for (int k = 1; k < horizon; k++) {
    SX delta_controls_val =
        u(Slice(12 * k, 12 * (k + 1))) - u(Slice(12 * (k - 1), 12 * k));
    f = f + SX::mtimes({delta_controls_val.T(), weightS, delta_controls_val});
  }
  ...
  for (int k = 0; k < horizon; k++) {
    for (int leg = 0; leg < 4; leg++) {
      if (k == 0 && leg == 0) {
        constraints =
            SX::vertcat({u(0 + leg * 3 + 12 * k) - mu * u(2 + leg * 3 + 12 * k),
                         u(1 + leg * 3 + 12 * k) - mu * u(2 + leg * 3 + 12 * k),
                         u(0 + leg * 3 + 12 * k) + mu * u(2 + leg * 3 + 12 * k),
                         u(1 + leg * 3 + 12 * k) + mu * u(2 + leg * 3 + 12 * k),
                         u(2 + leg * 3 + 12 * k)});
      } else {
        constraints =
            SX::vertcat({constraints,
                         u(0 + leg * 3 + 12 * k) - mu * u(2 + leg * 3 + 12 * k),
                         u(1 + leg * 3 + 12 * k) - mu * u(2 + leg * 3 + 12 * k),
                         u(0 + leg * 3 + 12 * k) + mu * u(2 + leg * 3 + 12 * k),
                         u(1 + leg * 3 + 12 * k) + mu * u(2 + leg * 3 + 12 * k),
  ...
  //构建求解器
  SXDict nlp_prob = {{"x", u}, {"p", p}, {"f", f}, {"g", constraints}};
...
void nmpc_caller::opti_solution(Eigen::Matrix<float, 12, 1> current_states,
  ...
  vector<float> res_all(res.at("x"));
  for (int i = 0; i < horizon; i++) {
    for (int j = 0; j < 12; j++) {
      // predict_trajectory(i * 12 + j) = res_all.at(f_num + i * 12 + j);
      control_command(i * 12 + j) = res_all.at(i * 12 + j);
    }
  }
}
